...
def register(instance):
    client_name = 'mastodon token bot'
    instance = instance
    if instance[:5] != 'https':
    ...
    import webbrowser
    webbrowser.open(instance+'/oauth/authorize?client_id='+client_id +'&redirect_uri=urn:ietf:wg:oauth:2.0:oob&response_type=code&scope=read%20write')
    ...
    user = dict()
    # get username
    username = json.loads(requests.get(instance+'/api/v1/accounts/verify_credentials',headers={'Authorization': 'Bearer '+access_token}).content.decode('utf-8'))['username']
...
def retrieve(username, instance):
    ...
    access_token = ''
    with open('/home/canor/scripts/twtimelinebot/cred.json') as f:
        cred = json.load(f)
    for i in cred:
